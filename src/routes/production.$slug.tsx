...
export const Route = createFileRoute("/production/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        { title: it.seo_title ?? `${it.title} — производство в Минске | event-hub.by` },
        { name: "description", content: it.seo_description ?? it.short_description ?? `Изготовление «${it.title}» под мероприятие в Минске. Сроки и доставка от event-hub.by.` },
