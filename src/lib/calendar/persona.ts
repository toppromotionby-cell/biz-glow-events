...
const CHANNEL_HINT: Record<AssistantChannel, string> = {
  telegram:
    "Канал — Telegram. Разрешены только теги <b>, <i>, <code>, <a>; пиши их как есть, не экранируй (&lt;b&gt; — ошибка). " +
    "Никакого Markdown (**, ##, ---) и таблиц символами. Блоки начинай строкой-заголовком с эмодзи: " +
    "«📅 <b>Встречи</b>», «✅ <b>Задачи</b>», «⏰ <b>Напоминания</b>». Пункты — со знака «•», время выделяй жирным.",
  voice:
...
/** Правила режима «сначала план»: собрать, предложить, дождаться утверждения. */
export const PLAN_MODE_RULES: string[] = [
  "Ничего не создавай, не меняй и не удаляй прямо сейчас — только предложи план.",
  "Сначала опирайся на календарь, задачи и память владельца; внешние источники — только если он явно попросил поискать.",
  "Каждый шаг должен быть конкретным и выполнимым одним разрешённым инструментом.",
  "Не придумывай дела, о которых не просили: минимум необходимого, ничего лишнего.",
  "Если данных не хватает — задай не больше трёх коротких уточняющих вопросов вместо выдуманных шагов.",
  "В summary можешь посоветовать внешний AI-сервис, но не считай это шагом плана.",
];

/** Фразы, по которым владелец включает режим плана. */
export function wantsPlanMode(text: string): boolean {
  const t = text.trim().toLowerCase();
  if (t.startsWith("/plan") || t.startsWith("/план")) return true;
  return /(подумай|предложи план|составь план|спланируй|продумай|набросай план|как лучше организовать|дай варианты)/i.test(t);
}
