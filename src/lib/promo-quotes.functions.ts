  ...
  .handler(async ({ data, context }): Promise<{ id: string }> => {
    ...
    if (data.fromId) {
      ...
      const rows = ((srcItems ?? []) as Record<string, unknown>[]).map((it, i) => ({
        ...
        cost_mode: it.cost_mode === "percent" ? "percent" : "amount",
        cost_input: Number(it.cost_input ?? it.cost ?? 0),
        note: String(it.note ?? ""),
    ...
    const rows = preset.items.map((it, i) => ({
      ...
      price: it.price ?? 0,
      cost: it.cost ?? 0,
      cost_mode: (it as { cost_mode?: string }).cost_mode === "percent" ? "percent" : "amount",
      cost_input: Number((it as { cost_input?: number }).cost_input ?? it.cost ?? 0),
      note: it.note ?? "",
      includes: normalizeIncludes(it.includes),
      exclude_from_commission: it.exclude_from_commission === true,
      included: it.included !== false,
...
export const restorePromoVersion = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ ok: true }> => {
    ...
    const items = (row.snapshot.items ?? []).map(normalizePromoItem).map((it, i) => ({
      ...
      price: it.price,
      cost: it.cost,
      cost_mode: it.cost_mode,
      cost_input: it.cost_input,
      note: it.note,
      includes: it.includes,
      exclude_from_commission: it.exclude_from_commission,
      included: it.included,
