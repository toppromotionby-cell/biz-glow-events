...
export function contentWidthPt(blank: PwBlank): number {
  return PAGE_W_PT - blank.marginXMm * 2 * MM;
}
...
export function estimateContentHeightPt(blocks: PwBlock[], blank: PwBlank): number {
  const w = contentWidthPt(blank);
...
export function availableHeightPt(blank: PwBlank): number {
  return PAGE_H_PT - (blank.marginTopMm + blank.marginBottomMm) * MM;
}
 ...
 */
export function pickFitFactor(blocks: PwBlock[], blank: PwBlank): number {
  if (blank.fitOnePage === false) return 1;
  const need = estimateContentHeightPt(blocks, blank);
  const avail = availableHeightPt(blank);
...
export function fittedBlank(blocks: PwBlock[], blank: PwBlank): PwBlank {
  return shrinkBlank(blank, pickFitFactor(blocks, blank));
}
