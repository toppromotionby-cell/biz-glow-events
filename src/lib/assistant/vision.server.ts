// Разбор скриншотов и картинок-документов, присланных в бота-помощника. Только сервер.
...
export async function analyzeAttachments(opts: {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[assistant-vision] ${res.status}: ${body.slice(0, 300)}`);
    ...
    console.error("[assistant-vision] failed", e instanceof Error ? e.message : e);
