...
const TEACH_PREFIX = /^\s*(?:бот[,:\s]+)?(?:запомни|запиши|учти|заруби на носу)(?![\p{L}])[\s:,—-]*/iu;
const ALWAYS = /(?<![\p{L}])(всегда|никогда|по умолчанию|каждый раз|обязательно)(?![\p{L}])/iu;
const ALIAS = /^\s*(.{2,60}?)\s+(?:это|—|-|означает)\s+(.{2,200})\s*$/i;
const FORGET = /^\s*(?:забудь|удали из памяти|не помни)(?![\p{L}])(?:\s+(?:про|о|об))?\s*[:,—-]?\s*(.{2,120})\s*$/iu;
...
export function detectTeaching(text: string): LearnCandidate | null {
  ...
  if (!explicit && /^(сделай|покажи|найди|создай|перенеси|удали|напомни|отправь|скинь|что|когда|где|кто|сколько|как)(?![\p{L}])/iu.test(body)) {
    return null;
  }
