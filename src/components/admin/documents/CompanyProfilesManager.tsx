    ...
    <div className="grid gap-4 lg:grid-cols-[260px_1fr]">
      ...
      <div className="space-y-2">
        {(companies ?? []).map((c) => (
          <button
            ...
            className={`w-full rounded-xl border p-3 text-left transition ${
              c.id === activeId ? "border-primary/60 bg-primary/5" : "border-border/60 hover:bg-muted/40"
            }`}
          >
            <div className="flex items-center gap-2">
              <Building2 className="h-4 w-4 shrink-0 text-muted-foreground" />
              <span className="truncate text-sm font-medium">{c.name}</span>
              {c.is_default && <Badge variant="secondary" className="ml-auto text-[10px]">основная</Badge>}
            </div>
        ...
        <Button type="button" variant="outline" className="w-full" onClick={addNew}>
          <Plus className="mr-1 h-4 w-4" />Добавить компанию
        </Button>
        {!companies?.length && (
      ...
      {!draft ? (
        ...
        <div className="space-y-5 rounded-xl border border-border/60 p-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <F label="Название профиля (видно только вам)" value={draft.name} onChange={(v) => set("name", v)} />
            <F label="Юр. название" value={draft.company_legal_name} onChange={(v) => set("company_legal_name", v)} />
            <F label="Бренд (для шапки)" value={draft.company_brand} onChange={(v) => set("company_brand", v)} />
            <F label="УНП" value={draft.company_unp} onChange={(v) => set("company_unp", v)} />
            <F label="Юридический адрес" value={draft.company_address} onChange={(v) => set("company_address", v)} />
            <F label="Телефон" value={draft.company_phone} onChange={(v) => set("company_phone", v)} />
            <F label="E-mail" value={draft.company_email} onChange={(v) => set("company_email", v)} />
            <F label="Сайт" value={draft.company_website} onChange={(v) => set("company_website", v)} />
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <F label="Расчётный счёт (IBAN)" value={draft.bank_account} onChange={(v) => set("bank_account", v)} />
          </div>

          <div className="grid gap-4 sm:grid-cols-3">
            <F label="Подписант" value={draft.signer_name} onChange={(v) => set("signer_name", v)} />
            <F label="Должность" value={draft.signer_title} onChange={(v) => set("signer_title", v)} />
            <F label="Действует на основании" value={draft.signer_basis} onChange={(v) => set("signer_basis", v)} />
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <LogoUploader label="Подпись" value={draft.signature_url} onChange={(v) => set("signature_url", v)} />
            <LogoUploader label="Печать" value={draft.stamp_url} onChange={(v) => set("stamp_url", v)} />
          </div>

          <div className="space-y-2">
            <Label className="text-xs text-muted-foreground">Акцентный цвет</Label>
            <div className="flex items-center gap-2">
              <Input value={draft.accent_color} onChange={(e) => set("accent_color", e.target.value)} placeholder="#FF7500" />
              <div className="flex items-center gap-1.5">
                <div className="h-9 w-9 rounded-md border border-border/60" style={{ background: draft.accent_color }} aria-hidden />
                {BRAND_ACCENTS.map((c) => (
                  <button key={c.hex} type="button" title={`${c.label} ${c.hex}`} onClick={() => set("accent_color", c.hex)}
          ...
          />
          <F label="Примечание об НДС" value={draft.vat_note} onChange={(v) => set("vat_note", v)} />
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-border/60 pt-4">
            ...
            <Button
              type="button"
              variant="outline"
              onClick={() => draft && save.mutate({ ...draft, id: activeId ?? undefined, is_default: !draft.is_default })}
              disabled={save.isPending}
            >
              {draft.is_default ? <StarOff className="mr-1 h-4 w-4" /> : <Star className="mr-1 h-4 w-4" />}
            ...
            {activeId && (
              <Button
                ...
                onClick={() => {
                  if (confirm("Удалить компанию? Документы с ней вернутся к общим настройкам.")) remove.mutate(activeId);
                }}
              >
