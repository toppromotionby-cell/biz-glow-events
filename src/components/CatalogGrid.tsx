    ...
    <>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-5">
          {filtered.map((it) => (
            <article key={it.slug} className="glass rounded-xl sm:rounded-2xl overflow-hidden group hover:border-primary/50 transition flex flex-col">
              <button
                type="button"
                    ...
                    {it.video ? (
                      <>
                        <video
                          ...
                          ref={(el) => {
                            if (!el) return;
                            const parent = el.closest("article") as HTMLElement | null;
                            if (!parent || parent.dataset.videoBound === "1") return;
              ...
              <div className="p-3.5 sm:p-4 lg:p-5 flex-1 flex flex-col">
                ...
                <p className="text-xs sm:text-sm text-muted-foreground mt-1.5 sm:mt-2 flex-1 line-clamp-2 sm:line-clamp-3">{it.description}</p>
                ...
                <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-border/40" data-category={category}>
                  <PriceGate>
                    <div className="flex items-baseline gap-1.5 flex-wrap">
                      ...
                      <span className="text-xl sm:text-2xl font-display font-bold gradient-text">{it.priceFrom.toLocaleString("ru-BY")}</span>
